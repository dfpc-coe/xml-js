import { isArray } from '../lib/array-helper.js';
...
export default function testItems(direction: string, options: any): Array<any> {
  let i, tests = [];
  options = options || {};
  ...
  function applyKeyNames(js) {
    ...
    for (key in options) {
      if (key.match(/Key$/)) {
        const keyName = (options.compact ? '_' : '') + key.replace('Key', '');
  ...
  function applyAttributesCallback(obj, key, parentKey) {
    if (options.nativeTypeAttributes) {
      const parsedNumber = Number(obj[key]);
  ...
  function applyValueCallbacks(value, key, parentKey) {
    ...
    for (fn in options) {
      if (fn.match(/Fn$/) && !fn.match(/NameFn$/)) {
        const callbackName = (options.compact ? '_' : '') + fn.replace('Fn', '');
